...
public final class InternalMethodHandler implements IInternalMethodHandler {
	...
	@Override
	@OnlyIn(Dist.CLIENT)
	public void renderTooltip(GuiGraphics guiGraphics, int x, int y, List<Component> tooltipData, int color, int color2, int width, int height) {
		if(!tooltipData.isEmpty()) {
			Font fontRenderer = Minecraft.getInstance().font;
			Screen screen = Minecraft.getInstance().screen;
			assert screen != null;
			guiGraphics.renderTooltip(fontRenderer, tooltipData, java.util.Optional.empty(), x, y);//TODO Fix color/color2? Is it needed?
